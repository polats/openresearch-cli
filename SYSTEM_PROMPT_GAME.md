...
6. **Never merge or rebase an experiment branch once it has a completed
   ...
   and play sessions came from — leave it as it ran. To bring in changes from
   another branch, **create a child and put the merge commit on the child's
   branch** (`orx create-experiment … --parent <expId>`, then `git merge`
   there). And never rebase, anywhere: the tree records what was actually
...
the wake-up to reconcile and continue the loop.) Play sessions are the
exception: the user plays on their own time — end your turn and pick up their
